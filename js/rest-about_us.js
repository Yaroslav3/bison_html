...
let extracted = function (data) {
    ...
    $.ajax({
        type: 'GET',
        url: 'http://admin.pristinebison.com/about_us/address',
        success: function (orders) {
            $.each(orders, function (i, data) {
                console.log(data.address);
                $('#google-map').attr('src', data.addressGoogle);
                $('#address-map').append(data.address);
            });
        }
    });
 ...
 const options = {
    ...
    let editor = new Quill('#editor-about_us', options);

    function getData() {

        $.ajax({
            url: 'http://admin.pristinebison.com/about_us/editor',
            dataType: 'json',
            type: 'GET',
            success: function (data) {
