$(document).ready(function() {
    ...
    $.ajax({
        type: 'GET',
        url: 'http://admin.pristinebison.com/photo_start',
        success: function (orders) {
            $.each(orders, function (i, data) {
                $('#myImg' + i).attr('src', extracted(data));
                $('#myImgMob' + i).attr('src', extracted(data));
            });
        }
    });
    ...
    $.ajax({
        type: 'GET',
        url: 'http://www.admin.exampletest.info/slider_text',
        success: function (orders) {
            $.each(orders, function (i, data) {
                $('#slide' + ++i).append(data.sliderText);
            });
        }
    });
    ...
    $.ajax({
        type: 'GET',
        url: 'http://admin.pristinebison.com/find_text',
        success: function (orders) {
            $.each(orders, function (i, data) {
                $('#text' + i).append(data.text);
                $('#text' + i + '-mob').append(data.text);
            });
        }
    });
    ...
    $.ajax({
        ...
        success: function (orders) {
            $.each(orders, function (i, data) {
				...
				$('#frame'+i).attr('src', data.youtubeVideo);                
            });
        }
    });
